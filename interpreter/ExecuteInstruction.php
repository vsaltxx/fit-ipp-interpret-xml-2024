...
class ExecuteInstruction
{
     ...
     * @throws FrameAccessException
     */
    public function validateAndRetrieveSymbolForCompare(MemoryFrame $memoryFrame, FrameStack $frameStack, string $symbType, string $symbValue, string $opcode): array
    {
     ...
     * @throws OperandTypeException
     */
    public function validateAndRetrieveSymbUniversal(string $symb, string $symbType, FrameStack $frameStack, MemoryFrame $memoryFrame): int|bool|string|null
    {
        if ($symbType == 'var') {
            $frame = substr($symb, 0, 2);
            $name = substr($symb, 3);
